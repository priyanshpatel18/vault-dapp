...
describe('action', () => {
  ...
  beforeAll(async () => {
    ...
    // Create user's ATA
    userTokenAccount = await getOrCreateAssociatedTokenAccount(provider.connection, user, mint, user.publicKey, true)

    vaultTokenAccount = await getOrCreateAssociatedTokenAccount(provider.connection, user, mint, vaultPda, true)

    // Mint tokens to user's ATA
    await mintTo(provider.connection, user, mint, userTokenAccount.address, user.publicKey, 1_000_000)
  })
